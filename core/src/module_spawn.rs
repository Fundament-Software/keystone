pub mod module_spawn_process {
    ...
    mod tests {
        ...
        //////
        // Warning: the ReadWrapper and WriteWrappers were generated by GPT-4 and modified until it worked, so it's
        ...
        #[tokio::test]
        // Acting as hello world server, spawning a hello world client. Talks over ServiceSpawn's bytestreams
        async fn server_with_bytestreams() {
            println!("server with bytestreams start");
        ...
        #[tokio::test]
        // Acting as hello world server, spawning a hello world client. Talks over stdio directly.
        // Basically duplicates the module's code
        async fn server_without_bytestreams() {
            println!("server without bytestreams start");
            let e = task::LocalSet::new().run_until(async {
        ...
        // currently broken
        // because it can't progress past the bytestream loop.
        #[tokio::test]
        // Acting as hello world client, spawning a hello world server. Talks over ServiceSpawn's bytestreams
        async fn client_with_bytestreams() {
            println!("client with bytestreams start");
            let spawn_process_server = UnixProcessServiceSpawnImpl();
            let spawn_process_client: UnixProcessServiceSpawnClient = capnp_rpc::new_client(spawn_process_server);
            ...
            let e = task::LocalSet::new().run_until(async {
                ...
                println!("reply sent and acquired: {:?}", reply.get()?.get_reply()?.get_message());
        ...
        #[tokio::test]
        // Acting as hello world client, spawning a hello world server. Talks over stdio directly.
        // Basically duplicates the module's code
        async fn client_without_bytestreams() {
            println!("client without bytestreams start");
            let e = task::LocalSet::new().run_until(async {
                println!("modulespawn start");
                let mut child = Command::new("../target/debug/keystone-hello-world-module")
                    .args(&["server"])
