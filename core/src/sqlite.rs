...
use crate::sqlite_capnp::{
    ...
    r_o_table_ref, result_stream, select, sql_function, table, table_ref, update,
};
use crate::sqlite_capnp::{
            ...
            expr::Which::Literal(dbany) => match dbany?.which()? {
                ...
                d_b_any::Which::Pointer(pointer) => {
                    let response = pointer
                        .get_as_capability::<saveable::Client>()?
                        .save_request()
                        .send()
                        .promise
                        .await?;
...
async fn match_dbany<'a>(
    ...
    match dbany.which()? {
        ...
        d_b_any::Which::Integer(int) => {
            statement_and_params
                .sql_params
                .push(rusqlite::types::Value::Integer(int));
            statement_and_params
                .statement
                .push_str(format!("?{}", statement_and_params.sql_params.len()).as_str());
        }
        d_b_any::Which::Real(real) => {
            statement_and_params
                .sql_params
                .push(rusqlite::types::Value::Real(real));
            statement_and_params
                .statement
                .push_str(format!("?{}", statement_and_params.sql_params.len()).as_str());
        }
        d_b_any::Which::Text(text) => {
            statement_and_params
                .sql_params
                .push(rusqlite::types::Value::Text(text?.to_string()?));
            statement_and_params
                .statement
                .push_str(format!("?{}", statement_and_params.sql_params.len()).as_str());
        }
        d_b_any::Which::Blob(blob) => {
            statement_and_params
                .sql_params
                .push(rusqlite::types::Value::Blob(blob?.to_vec()));
            statement_and_params
                .statement
                .push_str(format!("?{}", statement_and_params.sql_params.len()).as_str());
        }
        d_b_any::Which::Pointer(pointer) => {
            ...
            statement_and_params
                .sql_params
                .push(rusqlite::types::Value::Blob(restore_key.to_vec()));
            statement_and_params
                .statement
                .push_str(format!("?{}", statement_and_params.sql_params.len()).as_str());
        }
...
async fn match_where<'a>(
    ...
    for expr in w_expr {
        match_expr(db, expr, statement_and_params).await?;
    }
    Ok(())
}
            ...
            if token != "," {
                if token == "?" {
                    ...
                    match bind {
                        ...
                        Bindings::Column(c) => {
                            returning.push(expr::Expr::_Tablereference(
        ...
        while let Some(next) = iter.next() {
            if next == "?" {
                ...
                .clone()
                {
                    ...
                    Bindings::Column(c) => {
                        where_clause.push(expr::Expr::_Tablereference(
                            expr::table_column::TableColumn {
                                _col_name: c.name.clone(),
                                _reference: c.debruinin_level,
                            },
        ...
        while let Some(next) = iter.next() {
            if next == "?" {
                ...
                .clone()
                {
                    ...
                    Bindings::Column(c) => {
                        selectcore._sql_where.push(expr::Expr::_Tablereference(
                            expr::table_column::TableColumn {
                                _col_name: c.name.clone(),
                                _reference: c.debruinin_level,
                            },
    ...
    async fn test_test() -> eyre::Result<()> {
        ...
        let update_request = client
            ...
            .build_update_request(Some(update::Update {
                ...
                _sql_where: vec![
                    expr::Expr::_Tablereference(expr::table_column::TableColumn {
